...
{
    internal class DataAccessPolicyProvider : IAuthorizationPolicyProvider
    {
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public DataAccessPolicyProvider(IOptions<AuthorizationOptions> options)
        {
        ...
        // (like [DataAccessAuthorize] in this sample)
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(PolicyConstant.DataAccessPrefix, StringComparison.OrdinalIgnoreCase))
            {
                var dataAccessString = policyName.Substring(PolicyConstant.DataAccessPrefix.Length);
                var splitString = dataAccessString.Split('.');
                var dataTableName = splitString[1];
